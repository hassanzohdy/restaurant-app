...
export default function OrderDetailsPage() {
  return (
    <>
      ...
      <Breadcrumb title={trans('orderDetails')} navItems={[{name: trans('orderDetails')}]}/>
      <div className="order-details pt-[60px] pb-[100px] max-lg:pb-[80px] max-sm:pb-[70px]">
        <div className="container overflow-hidden">
          <div className="details px-5 py-8 mb-5 border border-[#eee] rounded-lg max-sm:px-4 max-sm:py-4">
            {/* Top Details */}
            <OrderTopDetails orderNumber={orderDetails.orderNumber} orderDate={orderDetails.orderDate}/>
            {/* Order Menu */}
            <OrderMenu orderMenu={orderDetails.orderMenu}/>
            {/* Bottom Details */}
            <OrderBottomDetails totalPrice={orderDetails.totalPrice} address={orderDetails.address} paymentMethod={orderDetails.paymentMethod}/>
          </div>
          <BaseLink href={URLS.cart} variant="primary" className="float-right px-8 py-3 text-[16px] max-sm:w-full">{trans("reOrder")}</BaseLink>
        </div>
      </div>
    </>
  );
