...
function _OrdersPage() {
  return (
    <>
      ...
      <Breadcrumb title={trans('myOrders')} navItems={[{name: trans('myOrders')}]}/>
      <div className="orders pt-[60px] pb-[100px] max-lg:pb-[80px] max-sm:pb-[70px]">
        <div className="container">
          <h2 className="text-[24px] text-secondary-text font-bold mb-6 pb-5 border-b-2 border-dashed border-[#eee]">{trans("pastOrders")}</h2>
          <ul className="orders-list">
            {
              ordersList.map((order, index) => <OrdersListItem key={index} order={order}/>)
            }
          </ul>
        </div>
      </div>
